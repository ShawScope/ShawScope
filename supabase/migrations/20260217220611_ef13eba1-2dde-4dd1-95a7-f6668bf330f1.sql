...
-- Fix user_roles: drop restrictive policy and recreate as permissive
...
ON public.appointments
FOR SELECT
USING (is_admin() OR (profile_id IN (SELECT profiles.id FROM profiles WHERE profiles.user_id = auth.uid())) OR ((access_token)::text = ((current_setting('request.headers'::text, true))::json ->> 'x-access-token'::text)));
...
CREATE POLICY "Admins can insert available dates" ON public.available_dates FOR INSERT TO authenticated WITH CHECK (is_admin());
DROP POLICY IF EXISTS "Admins can update available dates" ON public.available_dates;
CREATE POLICY "Admins can update available dates" ON public.available_dates FOR UPDATE TO authenticated USING (is_admin());
DROP POLICY IF EXISTS "Admins can delete available dates" ON public.available_dates;
CREATE POLICY "Admins can delete available dates" ON public.available_dates FOR DELETE TO authenticated USING (is_admin());
...
CREATE POLICY "Admins can insert services" ON public.services FOR INSERT TO authenticated WITH CHECK (is_admin());
DROP POLICY IF EXISTS "Admins can update services" ON public.services;
CREATE POLICY "Admins can update services" ON public.services FOR UPDATE TO authenticated USING (is_admin());
DROP POLICY IF EXISTS "Admins can delete services" ON public.services;
CREATE POLICY "Admins can delete services" ON public.services FOR DELETE TO authenticated USING (is_admin());
...
CREATE POLICY "Admins can insert templates" ON public.consent_form_templates FOR INSERT TO authenticated WITH CHECK (is_admin());
DROP POLICY IF EXISTS "Admins can update templates" ON public.consent_form_templates;
CREATE POLICY "Admins can update templates" ON public.consent_form_templates FOR UPDATE TO authenticated USING (is_admin());
DROP POLICY IF EXISTS "Admins can delete templates" ON public.consent_form_templates;
CREATE POLICY "Admins can delete templates" ON public.consent_form_templates FOR DELETE TO authenticated USING (is_admin());

-- Fix consent_form_responses
DROP POLICY IF EXISTS "Admins can manage consent responses" ON public.consent_form_responses;
CREATE POLICY "Admins can manage consent responses" ON public.consent_form_responses FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "Secure consent response access" ON public.consent_form_responses;
CREATE POLICY "Secure consent response access" ON public.consent_form_responses FOR SELECT USING (is_admin() OR (appointment_id IN (SELECT appointments.id FROM appointments WHERE appointments.profile_id IN (SELECT profiles.id FROM profiles WHERE profiles.user_id = auth.uid()))));
DROP POLICY IF EXISTS "Validated consent response insert" ON public.consent_form_responses;
CREATE POLICY "Validated consent response insert" ON public.consent_form_responses FOR INSERT WITH CHECK (is_admin() OR (EXISTS (SELECT 1 FROM appointments WHERE appointments.id = consent_form_responses.appointment_id)));

-- Fix email_templates
DROP POLICY IF EXISTS "Admins can manage email templates" ON public.email_templates;
CREATE POLICY "Admins can manage email templates" ON public.email_templates FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());

-- Fix patient_files
DROP POLICY IF EXISTS "Admins can manage patient files" ON public.patient_files;
CREATE POLICY "Admins can manage patient files" ON public.patient_files FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
...
CREATE POLICY "Admins can manage profiles" ON public.profiles FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
